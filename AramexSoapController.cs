...
public class AramexSoapController : ControllerBase
{
    ...
    private (DateTime PickupDate, DateTime ReadyTime, DateTime LastPickupTime, DateTime ClosingTime) GetDefaultPickupTimes()
    {
        ...
        // Skip non-working days (Friday and Saturday in Oman/Middle East)
        // Keep advancing until we find a working day (Sunday-Thursday)
        while (pickupDate.DayOfWeek == DayOfWeek.Friday || pickupDate.DayOfWeek == DayOfWeek.Saturday)
        {
            pickupDate = pickupDate.AddDays(1);
            _logger.LogInformation("Skipping non-working day, moving pickup to {PickupDate}", pickupDate.ToString("yyyy-MM-dd"));
        }
    ...
    private static string NormalizeCountryCode(string? country)
    {
        ...
        return v.ToLowerInvariant().Replace(" ", "") switch
        {
            "oman" or "om"                 => "OM",
            "saudiarabia" or "ksa" or "sa" => "SA",
            "unitedarabemirates" or "uae"  => "AE",
            "kuwait" or "kw"               => "KW",
            "bahrain" or "bh"              => "BH",
            "qatar" or "qa"                => "QA",
            _                              => v.ToUpperInvariant()[..Math.Min(2, v.Length)]
        };
    }
            ...
            string? transactionReference)
    {
        try
        {
            ...
            var consigneeAddress = requestShipment.Consignee?.PartyAddress;

            var isOmanDomestic = string.Equals(shipperAddress?.CountryCode?.Trim(), "OM", StringComparison.OrdinalIgnoreCase)
                && string.Equals(consigneeAddress?.CountryCode?.Trim(), "OM", StringComparison.OrdinalIgnoreCase);
            ...
            var pickup = new Services.Aramex.Shipping.Pickup
            {
                ...
                PickupItems = new[]
                {
                    ...
                    {
                        ProductGroup = isOmanDomestic ? "DOM" : (details?.ProductGroup ?? "EXP"),
                        ProductType = isOmanDomestic ? "OND" : (details?.ProductType ?? "PPX"),
                        NumberOfShipments = 1,
            ...
            {
                var isEmptyGuid = response.ProcessedPickup.GUID == Guid.Empty;
                if (isEmptyGuid)
                {
                    _logger.LogWarning("Pickup registered but no pickup GUID returned (ID={PickupId}). This is expected for some Oman domestic shipments.",
                        response.ProcessedPickup.ID);
                }
                else
                {
                    _logger.LogInformation("Pickup created successfully: ID={PickupId}, GUID={PickupGuid}",
                        response.ProcessedPickup.ID, response.ProcessedPickup.GUID);
                }
            ...
            _logger.LogWarning("Pickup creation failed: {Errors}", string.Join(", ", errors));
    ...
    public async Task<IActionResult> CreatePickup([FromBody] CreatePickupRequestDto request)
    {
        try
        {
            ...
            var pickup = new Services.Aramex.Shipping.Pickup
            {
                ...
                Status = string.IsNullOrWhiteSpace(request.Status) ? "Ready" : request.Status,
    ...
    public async Task<IActionResult> CreateShipment([FromBody] SimpleShipmentRequest request)
    {
        try
        {
            ...
            // Parse shipping date from payload or default to today
            var shippingDate = DateTime.Today;
            if (!string.IsNullOrWhiteSpace(request.Details.ScheduledPickup)
                && DateTime.TryParse(request.Details.ScheduledPickup, out var parsedDate))
            {
                shippingDate = parsedDate;
            }
            while (shippingDate.DayOfWeek == DayOfWeek.Friday || shippingDate.DayOfWeek == DayOfWeek.Saturday)
                shippingDate = shippingDate.AddDays(1);

            // Default GoodsOriginCountry to shipper's country if not provided
            var goodsOriginCountry = !string.IsNullOrWhiteSpace(request.Details.GoodsOriginCountry)
                ? request.Details.GoodsOriginCountry
                : request.Shipper.PartyAddress.CountryCode;

            var chargeableWeight = request.Details.ChargeableWeight ?? request.Details.ActualWeight;

            var shipments = new[]
            {
                new Services.Aramex.Shipping.Shipment
                {
                    Reference1 = request.Details.Reference1,
                    Reference2 = request.Details.Reference2,
                    ...
                    Details = new Services.Aramex.Shipping.ShipmentDetails
                    {
                        ...
                        PaymentOptions = request.Details.PaymentOptions ?? string.Empty,
                        ...
                        ChargeableWeight = new Services.Aramex.Shipping.Weight
                        {
                            Unit = chargeableWeight.Unit,
                            Value = chargeableWeight.Value
                        },
                        Dimensions = request.Details.Dimensions == null ? new Services.Aramex.Shipping.Dimensions
                        {
                            Length = 10, Width = 10, Height = 10, Unit = "CM"
                        } : new Services.Aramex.Shipping.Dimensions
                        {
                            Length = request.Details.Dimensions.Length,
                            Width = request.Details.Dimensions.Width,
                            Height = request.Details.Dimensions.Height,
                            Unit = request.Details.Dimensions.Unit
                        },
                        NumberOfPieces = request.Details.NumberOfPieces,
                        DescriptionOfGoods = request.Details.DescriptionOfGoods ?? "Goods",
                        CustomsValueAmount = request.Details.CustomsValueAmount == null ? null
                            : new Services.Aramex.Shipping.Money
                            {
                                CurrencyCode = request.Details.CustomsValueAmount.CurrencyCode,
                                Value = request.Details.CustomsValueAmount.Value
                            }
                    }
                }
            };

            var transaction = new Services.Aramex.Shipping.Transaction { Reference1 = request.Details.Reference1 ?? "WebOrder" };
            ...
            if (errors.Count == 0)
                errors.AddRange(response.Notifications?.Select(n => $"[{n.Code}] {n.Message}")
                    ?? new[] { "Shipment creation failed" });
            return BadRequest(new { success = false, errors });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> CreateShipmentForOrder(int orderId, [FromServices] SpirithubcafeApi.Services.IOrderService orderService)
    {
        try
        {
            ...
            // Determine shipper and consignee countries
            var shipperCountryCode = _configuration["Aramex:CountryCode"] ?? "OM";
            var consigneeCountryCode = NormalizeCountryCode(order.IsGift ? (order.GiftRecipientCountry ?? order.Country) : order.Country);
            
            // Determine if domestic or international shipment
            var isDomestic = string.Equals(shipperCountryCode, consigneeCountryCode, StringComparison.OrdinalIgnoreCase);
            
            // Set ProductGroup and ProductType based on domestic/international
            string productGroup;
            string productType;
            
            if (isDomestic)
            {
                // Domestic shipment - use DOM/OND
                productGroup = "DOM";
                productType = "OND"; // Default domestic service (can be made configurable)
                _logger.LogInformation("Creating domestic shipment for order {OrderNumber} (Country: {Country})", 
                    order.OrderNumber, consigneeCountryCode);
            }
            else
            {
                // International shipment - use EXP/PPX
                productGroup = "EXP";
                productType = "PPX"; // Priority international service
                _logger.LogInformation("Creating international shipment for order {OrderNumber} (From: {FromCountry}, To: {ToCountry})", 
                    order.OrderNumber, shipperCountryCode, consigneeCountryCode);
            }
            ...
            if (response.Shipments?.Length > 0 && !string.IsNullOrEmpty(response.Shipments[0].ID))
            {
                ...
                _logger.LogInformation("Pickup result for order {OrderNumber}: Success={Success}, PickupID={PickupId}, PickupGUID={PickupGuid}",
                    order.OrderNumber, pickupResult.Success, pickupResult.ProcessedPickup?.ID, pickupResult.ProcessedPickup?.GUID);
                
                // Update order with tracking number, pickup info and status
                var rawPickupGuid = pickupResult.ProcessedPickup?.GUID;
                var updateShippingResult = await orderService.UpdateShippingInfoAsync(order.Id, new OrderShippingUpdateDto
                {
                    ...
                    PickupGUID = rawPickupGuid.HasValue && rawPickupGuid.Value != Guid.Empty ? rawPickupGuid.Value.ToString() : null,
                ...
                {
                    _logger.LogInformation("Successfully updated order {OrderNumber} with pickup info: PickupGUID={PickupGuid}, PickupRef={PickupRef}",
                        order.OrderNumber, pickupResult.ProcessedPickup?.GUID.ToString(), pickupResult.ProcessedPickup?.ID);
                }
                ...
                _logger.LogInformation("Shipment and pickup created for order {OrderNumber}: TrackingNumber={TrackingNumber}, PickupReference={PickupRef}", 
                    order.OrderNumber, trackingNumber, pickupResult.ProcessedPickup?.ID);
    ...
    public async Task<IActionResult> TrackShipment(string awbNumber)
    {
        try
        {
            ...
            _logger.LogInformation("Track {AWB}: HasErrors={HasErrors}, ResultsCount={Count}, NotificationsCount={NotifCount}",
                awbNumber, response.HasErrors, response.TrackingResults?.Count ?? 0,
                response.Notifications?.Length ?? 0);

            if (response.TrackingResults != null && response.TrackingResults.Count > 0)
            {
            ...
            // Build error list; fall back to a meaningful message when Aramex returns no notifications
            var errors = response.Notifications?.Where(n => !string.IsNullOrEmpty(n.Message))
            ...
            if (errors.Count == 0)
            {
                // Shipment may be too new — Aramex tracking typically activates within a few hours
                errors.Add($"No tracking events found for AWB {awbNumber}. The shipment may not be in the tracking system yet — please try again in a few hours.");
            }

            _logger.LogWarning("Tracking failed for {AWB}: {Errors}", awbNumber, string.Join("; ", errors));
    ...
    /// </summary>
    /// <param name="request">Request containing shipment number and optional product group.</param>
    ...
    /// <returns>Label PDF file.</returns>
...
#pragma warning restore S6962
    ...
    public async Task<IActionResult> CreateShipmentForOrder([FromBody] CreateShipmentForOrderRequest request)
    {
        try
        {
            ...
            var shipperPhone = _configuration["Aramex:PhoneNumber"] ?? "+96891900005";
            ...
            var recipientCity = order.IsGift && !string.IsNullOrEmpty(order.GiftRecipientCity) 
                ...
                : order.City;
            var recipientCountry = NormalizeCountryCode(order.IsGift && !string.IsNullOrEmpty(order.GiftRecipientCountry) 
            ...
            // Determine product group/type based on domestic vs international
            bool isDomestic = request.ShipmentMode?.ToUpperInvariant() switch
            {
                "DOMESTIC"      => true,
                "INTERNATIONAL" => false,
                _               => string.Equals(shipperCountry, recipientCountry, StringComparison.OrdinalIgnoreCase)
            };
            var productGroup = isDomestic ? "DOM" : "EXP";
            var productType = isDomestic ? "OND" : "PPX";

            // Determine shipping date (apply weekend-skip)
            var orderShippingDate = DateTime.Today;
            if (!string.IsNullOrWhiteSpace(request.ScheduledPickup)
                && DateTime.TryParse(request.ScheduledPickup, out var reqPickupDate))
            {
                orderShippingDate = reqPickupDate;
            }
            while (orderShippingDate.DayOfWeek == DayOfWeek.Friday || orderShippingDate.DayOfWeek == DayOfWeek.Saturday)
                orderShippingDate = orderShippingDate.AddDays(1);

            var shipments = new[]
            {
                new Services.Aramex.Shipping.Shipment
                {
                    ShippingDateTime = orderShippingDate,
                    Shipper = new Services.Aramex.Shipping.Party
                    {
                        AccountNumber = accountNumber,
                        PartyAddress = new Services.Aramex.Shipping.Address
                        ...
                        Contact = new Services.Aramex.Shipping.Contact
                        {
                            ...
                            EmailAddress = _configuration["Aramex:Email"] ?? "info@spirithubcafe.com"
                        }
                    },
                    Consignee = new Services.Aramex.Shipping.Party
                    {
                    ...
                    Details = new Services.Aramex.Shipping.ShipmentDetails
                    {
                        ...
                        GoodsOriginCountry = shipperCountry,
                        ActualWeight = new Services.Aramex.Shipping.Weight
                        {
                            Unit = "KG",
                            Value = totalWeight
                        ...
                        Dimensions = new Services.Aramex.Shipping.Dimensions
                        {
                            Length = 20, Width = 20, Height = 20, Unit = "CM"
                        },
                        NumberOfPieces = numberOfPieces,
                        DescriptionOfGoods = descriptionOfGoods,
                        CustomsValueAmount = isDomestic ? null : new Services.Aramex.Shipping.Money
                        {
                            CurrencyCode = GetPickupCurrencyCode(),
            ...
            if (response.Shipments?.Length > 0 && !string.IsNullOrEmpty(response.Shipments[0].ID))
            {
                ...
                _logger.LogInformation("Shipment created for Order {OrderId}: TrackingNumber={TrackingNumber}, PickupSuccess={PickupSuccess}, PickupID={PickupId}, PickupGUID={PickupGuid}", 
                    order.Id, trackingNumber, pickupResult.Success, pickupResult.ProcessedPickup?.ID, pickupResult.ProcessedPickup?.GUID);
                ...
                {
                    _logger.LogInformation("Successfully updated order {OrderId} with TrackingNumber={TrackingNumber}, PickupGUID={PickupGuid}, PickupReference={PickupRef}",
                        order.Id, trackingNumber, pickupResult.ProcessedPickup?.GUID.ToString(), pickupResult.ProcessedPickup?.ID);
                }

                var updateStatusResult = await _orderService.UpdateOrderStatusAsync(order.Id, "Shipped");
                if (!updateStatusResult.Success)
                {
                    _logger.LogWarning("Failed to update status for order {OrderId}: {Message}",
                        order.Id, updateStatusResult.Message);
                }
...
public class SimpleShipmentDetails
{
    ...
    /// <summary>Scheduled pickup/shipping date (yyyy-MM-dd). Defaults to today if omitted.</summary>
    public string? ScheduledPickup { get; set; }
    public SimpleMoney? CustomsValueAmount { get; set; }
}
...
public class CreateShipmentForOrderRequest
{
    ...
    public string? ShipmentMode { get; set; }    // AUTO | DOMESTIC | INTERNATIONAL
    public string? ScheduledPickup { get; set; } // yyyy-MM-dd
}
